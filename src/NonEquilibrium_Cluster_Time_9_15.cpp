...
int main(){
	...
	for(int k = 0 ; k < TotalTime_VectorSize ; k++){
		...
		PropogateProtocol(WorkArray, CPVals_Naive, LagTimes_Naive, NumCPVals_TimeTrend, Protocol_Repetitions, kL, kTrap);
		WriteWorkArray(WorkWrite_Naive,WorkArray,Protocol_Repetitions);
		PropogateProtocol(WorkArray, CPVals_TimeOpt, LagTimes_TimeOpt, NumCPVals_TimeTrend, Protocol_Repetitions, kL, kTrap);
		WriteWorkArray(WorkWrite_TimeOpt,WorkArray,Protocol_Repetitions);
		
		cout << "\nComplete\n";
		break;

		//PropogateProtocol(WorkArray, CPVals_FullOpt, LagTimes_FullOpt, NumCPVals_TimeTrend, Protocol_Repetitions, kL, kTrap);
